...
public class TTS_Controller {
	...
	public void ttsServer(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			log.info("request带来的数据 :" + requestParams);
...
//			response.setHeader("body", result);
			...
			//调用科大讯飞的tts引擎生成文件
			kdxtTts(kdxf_app_id, speech_url, paramMap);
			
		} catch (IOException e) {
			log.info("request传来的数据异常!!");
			e.printStackTrace();
		}
	}
	//百度的tts流程
	...
	//科大讯飞的tts流程
	private void kdxtTts(String kdxf_app_id, final String speech_url, final Map<String,String> paramMap) {
		...
		mTts.synthesizeToUri(paramMap.get("txt"), speech_url+"/"+yyyy+"/"+mm+"/"+dd+"/"+uuid+".pcm", synthesizeToUriListener);
